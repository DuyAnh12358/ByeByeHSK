...
export default function VocabularyHSK5() {
    ...
    async function fetchVocabularies() {
      ...
      try {
        const res = await fetch(
          `/api/vocabularies/level/hsk5?page=${page}&limit=20`,
          { signal: controller.signal },
        );
        const json = await res.json();

        if (!json.success) {
          throw new Error(json.message || "Không thể tải từ vựng HSK5.");
        }
  ...
  return (
    <HomeLayout>
      <PageTitle
        title="Từ vựng HSK5"
        subtitle="Danh sách từ vựng cấp độ HSK5 có sẵn"
      />
      ...
      <div className="px-4 py-6">
        ...
        {!loading && !error && vocabularies.length === 0 && (
          <p className="text-center text-base-content/60 py-10">
            Chưa có từ vựng nào cho cấp độ HSK5.
          </p>
        )}
