...
export default function VocabularyHSK4() {
    ...
    async function fetchVocabularies() {
      ...
      try {
        const res = await fetch(
          `/api/vocabularies/level/hsk4?page=${page}&limit=20`,
          { signal: controller.signal },
        );
        const json = await res.json();

        if (!json.success) {
          throw new Error(json.message || "Không thể tải từ vựng HSK4.");
        }
  ...
  return (
    <HomeLayout>
      <PageTitle
        title="Từ vựng HSK4"
        subtitle="Danh sách từ vựng cấp độ HSK4 có sẵn"
      />
      ...
      <div className="px-4 py-6">
        ...
        {!loading && !error && vocabularies.length === 0 && (
          <p className="text-center text-base-content/60 py-10">
            Chưa có từ vựng nào cho cấp độ HSK4.
          </p>
        )}
